...
public class UrnColoringDisplay extends BasicDisplay {
    public UrnColoringDisplay(DyeColor color) {
        super(getInputs(color), getOutputs(color));
    }

    @Override
    public CategoryIdentifier<?> getCategoryIdentifier() {
        return UrnColoringCategory.ID;
    }

    private static List<EntryIngredient> getInputs(DyeColor color) {
        List<EntryIngredient> inputs = new ArrayList<>();

        List<EntryStack<?>> allUrns = new ArrayList<>();
        allUrns.add(EntryStacks.of(new ItemStack(UrnBlock.get(null))));
        for (DyeColor c : DyeColor.values()) {
            allUrns.add(EntryStacks.of(new ItemStack(UrnBlock.get(c))));
        }

        inputs.add(EntryIngredient.of(allUrns));
        inputs.add(EntryIngredients.of(DyeItem.byColor(color)));
        return inputs;
    }

    private static List<EntryIngredient> getOutputs(DyeColor color) {
        return List.of(EntryIngredients.of(new ItemStack(UrnBlock.get(color))));
    }
}