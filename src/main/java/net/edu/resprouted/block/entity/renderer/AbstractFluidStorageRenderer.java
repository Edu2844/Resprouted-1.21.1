...
public abstract class AbstractFluidStorageRenderer<T extends FluidStorageBE> implements BlockEntityRenderer<T> {
    ...
    protected AbstractFluidStorageRenderer(BlockEntityRendererFactory.Context context,
                                           float minHeight, float heightMultiplier) {
        this.minHeight = minHeight;
        this.heightMultiplier = heightMultiplier;
    }

    static class SmoothFloat {
    ...
    public void render(T entity, float tickDelta, MatrixStack matrices,
        ...
        renderCustomContent(entity, tickDelta, matrices, vertexConsumers, light, overlay);

        renderFluid(entity, tickDelta, matrices, vertexConsumers, light);
    }
    protected abstract void renderCustomContent(T entity, float tickDelta, MatrixStack matrices,
                                                VertexConsumerProvider vertexConsumers, int light, int overlay);

    protected void renderFluid(T entity, float tickDelta, MatrixStack matrices,
        ...
        if (!fluid.isBlank() && amount > 0) {
            ...
            animation.update(targetHeight, 0.5f);
